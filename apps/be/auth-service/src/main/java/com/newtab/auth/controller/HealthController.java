...
public class HealthController {
    ...
    @Operation(
            ...
            description = "Checks if the service is running. Lightweight endpoint suitable for Kubernetes liveness probes. Does not check dependencies."
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
    ...
    public ResponseEntity<LivenessResponse> liveness() {
        LivenessResponse response = new LivenessResponse("UP", "auth-service");
    ...
    public record HealthResponse(
            @Schema(description = "Overall status of the service", example = "UP")
            String status,
            @Schema(description = "Name of the service", example = "auth-service")
            String service,
